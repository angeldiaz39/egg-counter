...
class Communicate(QObject):
	update_counst_signal = pyqtSignal(dict)
...
class CounterWindow(QWidget):
	def __init__(self):
		...
		self.communicate.update_counst_signal.connect(self.update_counts)
	def initUI(self):
		self.setWindowTitle('Contador de Huevos')
		self.layout = QVBoxLayout()
		self.startButton = QPushButton('Iniciar Contadores',self)
		self.startButton.clicked.connect(self.start_counters)
	...
	def start_counters(self):
		#self.tracker_threads=[]
		for i, (video_file, model_name, show) in enumerate(zip(SOURCES, MODEL_NAMES, SHOW_VID)):
			thread = threading.Thread(target=run_counter_in_thread, args=(model_name, video_file, trt_on, show, i, self.communicate))
			self.COUNTS[i]=0
			self.tracker_threads.append(thread)
			thread.start()
		'''
		for video_file, model_name, show in zip(SOURCES, MODEL_NAMES, SHOW_VID):
			thread = threading.Thread(target=run_counter_in_thread, args=(model_name, video_file, trt_on, show, int(len(self.tracker_threads))))
			COUNTS[int(len(self.tracker_threads))]=0
			self.tracker_threads.append(thread)
			thread.start()

		#Hilo para ir mostrando el conteo	
		self.show_thread=threading.Thread(target=self.update_counts)
	...
	def update_counts(self):
		while True:
			...
			counts_display = "\n".join([f'Cam-{i}: {count}' for i, count in self.COUNTS.items()])
			self.textEdit.setPlainText(counts_display)
			time.sleep(1)

def run_counter_in_thread(model, source, trt, show, num, communicator):
	# Load the model.
	model = YOLO(model)
